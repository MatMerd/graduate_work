...
from schemas.cinema_room import CinemaRoom, CinemaRoomCreate, CinemaRoomUpdate, CinemaRoomUserTypeEnum, User
...
class CinemaCRUD:
    ...
    def is_user_in_cinema_room(
        self, cinema_room: CinemaRoom, user: User
    ) -> bool:
        if (cinema_room.users and user.user_id in [u.user_id for u in cinema_room.users]):
            return True
    ...
    async def add_user_to_users_list(
        ...
        if not self.is_user_in_cinema_room(
            ...
            raise UserAlreadyExistError(f"User {user.username} already in room {cinema_room.cinema_room_key}")
